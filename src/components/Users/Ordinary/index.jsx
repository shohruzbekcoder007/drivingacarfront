...
export default function Ordinary() {
    ...
    const searchDevice = () => {
        setPage(1)
        axios
            .get(`${user_list}?status=3&name=${userName.current.value}&email=${userEmail.current.value}&tel_number=${userTelNumber.current.value}&device_number=${userDeviceNumber.current.value}`, {
                headers: {
    ...
    useEffect(() => {
        axios
            .get(`${user_list}?status=3&page=${page}`, {
                headers: {
                    "x-auth-token": sessionStorage.getItem("x-auth-token"),
                },
            })
        ...
        <>
            ...
            <Paper elevation={4} sx={{ margin: "15px 0", p: 2 }}>
                ...
                >
                    ...
                    <TextField id="standard-basic3" label="Device number" variant="standard" inputRef={userDeviceNumber} />
